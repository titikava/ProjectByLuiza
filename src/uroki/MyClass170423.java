...
try {
   ...
   *какой-то код, который обработает ошибку*
    ...
    *блок кода*
    ...
    Типы исключений
    Существует два типа исключений - проверяемые (checked) и непроверяемые (unchecked)
    Главным отличием является то, что checked проверяется во время компиляции, а unchecked во время запуска.
    (Смотреть потоки)
    Thread.sleep бросает исключение InterruptedException - это пример checked исключения. Ваш код не скомпилируется
    пока вы не обработаете исключения (try/catch).
    unchecked - деление на ноль или тип того.
*/
